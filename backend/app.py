...
class tblConcursos(db.Model):
    ...
    urlBanner = db.Column(db.String(50))
...
class tblConcursos_Schema(ma.Schema):
    class Meta:
        fields = ('id', 'nombre', 'url', 'precio', 'urlBanner', 'guion',
                  'recomendaciones', 'fechainicio', 'fechafin', 'fechacreacion')
    ...
    def put(self, id_tblConcursos):
        #def update_concurso(current_user,id_tblConcursos):
            ...
            if 'url' in request.json:
                concurso.url=request.json['frontEndUrl']
            if 'precio' in request.json:
                concurso.precio=request.json['precio']
            if 'urlBanner' in request.json:
                concurso.urlBanner=request.json['urlBanner']
...
class RecursoAgregarAdmins(Resource):
    ...
    def post(self):
        #def create_admin(current_user):
            ...
            nuevo_admin=tblAdministradores(nombre=nombre, apellido=apellido, email=email,password=hashed_password)
    ...
    def put(self, id_tblAdministradores):
        #def update_concurso(current_user,id_tblConcursos):
            ...
            if 'password' in request.json:
                concurso.guion=request.json['password']
